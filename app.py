...
def generate_response(user_input, conversation_chain, chain):
    ...
    with st.spinner('Spinning a snazzy reply...'):
        try:
            ...
            output = "I'm sorry, but I couldn't retrieve the information you're looking for. Please try asking something else."
...
def conversation_chat(user_input, conversation_chain, chain, history):
    ...
    returns:
    - result["answer"]: Response generated from LLM human readable format
    """
    ...
    if st.session_state['generated']:
        with reply_container:
            for i in range(len(st.session_state['generated'])):
                #emojis from "dicebear.com/styles"
                message(st.session_state["past"][i], is_user=True, key=f"{i}_user",avatar_style="micah",seed="aneka")
